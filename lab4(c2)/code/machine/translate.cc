...
// check for alignment errors
    ...
    //  ASSERT(tlb == NULL || pageTable == NULL);	
       ASSERT(tlb != NULL || pageTable != NULL);
    //we have to make sure tlb!=NULL or pagetable !=NULL   
  ...
  /*  if (tlb == NULL)
    ...
    {*/
		...
		{				// not found
    	   ...
    	   // printf("TLB MISS try with addr: %d vpn: %d\n",virtAddr,vpn);
    	    return PageFaultException;		// really, this is a TLB fault,
						// the page may be in memory,
      			// but not in the TLB
    	}*/
    	//printf("TLB HIT try with addr: %d  vpn:%d\n",virtAddr,vpn);
    //}
    	/*if (!entry->valid)
    	{
     ...
     /*for(i=0;i<NumPhysPages;i++)
        {
            if(machine->inverseTable[i].owner!=NULL)
            {
                machine->inverseTable[i].phy_lastuse+=1;
            }
        }
        machine->inverseTable[entry->physicalPage].phy_lastuse=0;*/
    int ppn=-1;
    for(int k=0;k<NumPhysPages;k++)
    {
        if(machine->inverseTable[k].owner==currentThread
            &&machine->inverseTable[k].vpn==vpn)
        ppn=k;    
    } 
    if(ppn==-1) // can not find a physical page
    return PageFaultException; 
    for(int k=0;k<NumPhysPages;k++)
    {
        if(MemoryMap->Test(k))
            machine->inverseTable[k].phy_lastuse+=1;
    } 
    machine->inverseTable[ppn].phy_lastuse=0;
    pageFrame=ppn; 
