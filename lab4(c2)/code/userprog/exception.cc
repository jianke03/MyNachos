...
NEW_LRU(int vpn)
{
	...
	if(emptyid>=0)
	{
		machine->inverseTable[emptyid].owner=currentThread;
		machine->inverseTable[emptyid].vpn=vpn;
		memcpy(machine->mainMemory+emptyid*PageSize,
			currentThread->space->swapArea+vpn*PageSize,PageSize);
		return TRUE;
	}
	else
	{
		...
		{
			if(machine->inverseTable[k].phy_lastuse>maxval)
			{
				replaceid=k;
				maxval=machine->inverseTable[k].phy_lastuse;
			}
		}
		int replacevpn=machine->inverseTable[replaceid].vpn;
		Thread* replaceThread=machine->inverseTable[replaceid].owner;
		memcpy(replaceThread->space->swapArea+replacevpn*PageSize,
			machine->mainMemory+replaceid*PageSize,PageSize);
		memcpy(machine->mainMemory+replaceid*PageSize,
			currentThread->space->swapArea+vpn*PageSize,PageSize);
		return TRUE;
	}
