...
int 
BitMap::FindFirst() //  find a bit which is 32's times first
{
    for (int i = 0; i < numBits; i=i+32)
    if (!Test(i))
    {
        Mark(i);
        printf("first allocate %d in track %d\n",i,i/32);
        return i;
    }
    int temp= Find();
    printf("first allocate %d sector in  track %d",temp,temp/32);
    return temp;
}

int
BitMap::FindNext(char firsttrack)
{
    int track= (int)firsttrack;
    for(int i=track*32;i<(track+1)*32;i++)
        if(!Test(i))
        {
            Mark(i);
            printf("allocate %d in track %d\n",i,track);
            return i;
        }
    int temp= Find();
    printf("first allocate %d sector in  track %d",temp,temp/32);
