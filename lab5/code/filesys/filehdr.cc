...
{ 
    ...
    for(int i=0;i<temp;i++)
        {
            if(i==0)
            {
                dataSectors[i] = freeMap->Find();//First(); 
                //firstSec = (char)(int)(dataSectors[i]/SectorsPerTrack);
            }
            else
            {
                dataSectors[i] = freeMap->Find();//Next(firstSec);
            }
            if(dataSectors[i]==-1)
                ASSERT(FALSE);
        }
    ...
    if(copyNumSectors>0)
    {
        dataSectors[9]=freeMap->Find();//Next(firstSec);
        int firstIndex[32];
        temp = min(32,copyNumSectors);
        for(int i=0;i<temp;i++)
        {
            firstIndex[i] = freeMap->Find();//Next(firstSec);
            if(firstIndex[i]==-1)
                ASSERT(FALSE);
        }
        copyNumSectors-=temp;
        ...
        {
            dataSectors[10] = freeMap->Find();//Next(firstSec); //save 2-level index
            if(dataSectors[10]==-1)
                ASSERT(FALSE);
            int id=0;
            int secondIndex[32];
            ...
            {
                secondIndex[id] = freeMap->Find();//Next(firstSec);
                if(secondIndex[id]==-1)
                    ASSERT(FALSE);
                int lowIndex[32];
                temp = min(32,copyNumSectors);
                for(int i=0;i<temp;i++)
                {
                    ...
                    lowIndex[i]= freeMap->Find();//Next(firstSec);
                    if(lowIndex[i]==-1)
                        ASSERT(FALSE);
                }
