...
void Readconsole(char *MyName)
{
   OpenFile *openFile = fileSystem->Open(MyName);
   while(TRUE)
  {
    empty->P();
    bool result= openFile->ReadFromConsole();
    full->V();
    if(result)
        break;
  } 
}
void Writeconsole(char *MyName)
{
    OpenFile *openFile = fileSystem->Open(MyName);
    while(TRUE)
    {
        full->P();
        bool result = openFile->WriteToConsole();
        empty->V();
        if(result)
        {
            delete synchConsole;
...
void TestForDirectEX(char* from ,char * to)
{
    ...
    t1->Fork(Readconsole,to);
    t2->Fork(Writeconsole,to);    
...
void TestForConsole(char *in, char *out)
{
    ...
    for (;;) {
    ch = synchConsole->Rchar();
    synchConsole->Wchar(ch);   // echo it!
