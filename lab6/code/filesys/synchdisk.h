...
class SynchDisk {
  public:
    ...
    void W_R_V(int sector);
    void W_R_P(int sector);
    Semaphore* remove_sema[NumSectors];
    int openNum[NumSectors];
    bool ifWait[NumSectors];
   private:
    ...
    Semaphore* sector_sema[NumSectors];
};
